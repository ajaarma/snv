...
sub new {
  ...
  while(<$fh>) {
    chomp;
    my ($gene, $pLI, $pRec, $miss_z) = split;
    next if $pLI eq 'pLI';
    $scores{lc($gene)} = sprintf("%.2f", $pLI) . '&' . sprintf("%.2f", $pRec) . '&' . sprintf("%.2f", $miss_z);
  }
...
sub get_header_info {
  return {
    Constraint => "pLI;pRec;miss_z values for gene"
  };
}
...
sub run {
 ...
 return $self->{scores}->{lc($symbol)} ? { Constraint => $self->{scores}->{lc($symbol)}} : {};
}
