...
=head1 DESCRIPTION
 ...
 Four additional terms may be added:
 ...
 # splice_donor_region_variant : variant falls in region between 3rd and 6th base after splice junction or the last two bases of the exon (5' end of intron)

       vv  vvvv
 ...EEEEEIIIIIIIIII...

 # splice_polypyrimidine_tract_variant : variant falls in polypyrimidine tract at 3' end of intron, between 25 and 3 bases from the end

 ...vvvvvvvvvvvvvvvvvv
 ...IIIIIIIIIIIIIIIIIIIIEEEEE...

 # splice_donor_0th_base_variant : variant falls in the last base of the exon (5' end of intron)
...
my %TERM_RANK = (
  ...
  splice_donor_0th_base_variant => 2,
  splice_donor_region_variant => 3,
  splice_polypyrimidine_tract_variant => 4,
  extended_intronic_splice_region_variant_5prime => 5,
  extended_intronic_splice_region_variant_3prime => 6,
);
...
sub run {
    ...
    @terms = (
      ...
      },
     {
      ...
      },
      {
        ...
        region => [$intron->{$acc_coord} + (-24 * $strand_mod), $intron->{$acc_coord} + (-2 * $strand_mod)],
        # allele_specific_mod => {
        #   A => '_to_purine',
        #   G => '_to_purine',
        # }
  ...
  for my $exon(@{$tv->_overlapped_exons($vf_start, $vf_end)}) {

    # define terms to check for and their regions
    @terms = (
      {
        term => 'splice_donor_0th_base_variant',
        region => [$exon->{$acc_coord}, $exon->{$acc_coord}]
      },
      {
        term => 'splice_donor_region_variant',
        region => [$exon->{$acc_coord} + (-1 * $strand_mod), $exon->{$acc_coord}]
      },
    );
